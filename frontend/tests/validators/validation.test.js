...
describe("Validators", () => {
  describe("Login form:", () => {
    test("If username string is empty, return a validation object with appropriate validation message", () => {
      const result = validateLoginForm("", "testPassword");

      expect(result).toEqual({
        username: "Username address is required",
      });
    });
    ...
    test("If username and password strings are empty, return a validation object with appropriate validation message", () => {
    ...
    test("If username and password strings are valid, return an empty validation object", () => {
      const result = validateLoginForm("test@test.com", "testPassword");

      expect(result).toEqual({});
    });

    test("If username isn't a string, it returns an error message", () => {
      const result = validateLoginForm(1, "abc");

      expect(result).toEqual({
        general: "Username and password must be string.",
      });
    });
